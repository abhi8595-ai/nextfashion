...
<template>
  <div class="flex flex-wrap justify-center max-w-screen-2xl m-auto">
    <TransitionGroup
      ...
      tag="div"
      mode="in-out"
      class="w-full flex flex-wrap justify-center max-w-screen-2xl m-auto">
      <article
        v-for="product in (wishlist || []).filter(Boolean)"
        ...
        <div class="relative overflow-hidden pb-[125%] rounded-[32px]">
          ...
          <NuxtLink
            ...
            <div class="grid gap-0.5 text-white">
              ...
              <div class="text-sm font-medium">{{ safeProduct(product).allPaStyle?.name || '' }}</div>
            </div>
          </NuxtLink>
        </div>
