...
async function fetch() {
  try {
    ...
    searchResults.value = Array.isArray(response?.products?.nodes) ? response.products.nodes : [];
  } catch (error) {
  ...
  <header class="relative z-50 border-b border-black/5 bg-white/95 shadow-[0_8px_30px_rgba(0,0,0,0.06)] backdrop-blur-xl dark:border-white/10 dark:bg-black/95">
    <div class="mx-auto flex w-full max-w-7xl flex-col">
      ...
      <NuxtLink
        ...
        :to="localePath('/')">
        <div class="flex items-center justify-center rounded-lg max-lg:rounded-full bg-black w-8 h-8 text-white font-black text-lg">
          9
        </div>
      </NuxtLink>
      <NuxtLink
      ...
      <div v-else class="mx-auto p-3 lg:p-4 max-w-screen-2xl">
        ...
        <div class="grid grid-cols-1 xs:grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-3 lg:gap-5 mt-3 lg:mt-5">
          <NuxtLink
            ...
            v-for="(product, i) in (searchResults || []).filter(Boolean)"
            :key="i"
            ...
            <div class="cursor-pointer transition ease-[ease] duration-300">
              <div class="relative pb-[133%] dark:shadow-[0_8px_24px_rgba(0,0,0,.5)] rounded-2xl overflow-hidden">
                <img
                  ...
                  :src="safeProduct(product).galleryImages?.sourceUrl || safeProduct(product).image?.sourceUrl || '/logo.svg'"
                  class="absolute h-full w-full dark:bg-neutral-800 bg-neutral-200 object-cover" />
                <img
                  :alt="product?.name || 'Product image'"
                  loading="lazy"
                  :title="product?.name || 'Product'"
                  :src="safeProduct(product).image?.sourceUrl || safeProduct(product).galleryImages?.sourceUrl || '/logo.svg'"
